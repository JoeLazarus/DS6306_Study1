...
```{r}
max.abv <- beerbrew %>% filter(ABV==max(beerbrew$ABV,na.rm=TRUE)) %>% select(Beer_Name,ABV,Class,Style,State)
max.abv
#Answer Colorado, Lee Hill Series Vol.5, ABV = 0.128

max.ibu <- beerbrew %>% filter(IBU==max(beerbrew$IBU,na.rm=TRUE)) %>% select(Beer_Name,ABV,Class,Style,State)
max.ibu
