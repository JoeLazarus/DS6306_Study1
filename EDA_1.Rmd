...
#bar chart of top 8 most advantagous ratio
brew.consum %>% ggplot(aes(x=reorder(filteredstate, -per.capita.brewery.strength), y = per.capita.brewery.strength)) +
  geom_bar(aes(fill= filteredstate),stat = "identity") +
  labs(title = "Top 8 States. Number of breweries compared to per capita consumption", 
       x="Top 8 states (other represents all other states combined)", y="Num Breweries / Per capita consumption")
...
#data prep need to remove column named filtered states column for the merge
brew.consum <- subset(brew.consum, select = -c(filteredstate))
